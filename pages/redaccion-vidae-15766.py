...
st.set_page_config(
    page_title="Dashboard Vidae - Redacción",
    page_icon="💻",
    layout="wide"
)

# Verificar login antes de mostrar contenido
if not check_login('vidae', page_type='redaccion'):
    st.stop()

# Obtener configuración del medio
media_config = create_media_config()['vidae']
    ...
    **Posibles causas:**
    ...
    if not sheets_filtered.empty and ga4_df is not None and not ga4_df.empty:
        ...
        with tab1:
            ...
            st.markdown(f"""
            ### 🎯 Objetivo del Mes
            **Meta:** 1,000,000 de Page Views{' desde ' + country_filter if country_filter != "Todos los países" else ''}
            ...
            monthly_goal = 1000000  # 1 millón de Page Views
        ...
        with tab2:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"vidae_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv"
            )
        ...
        with tab_crecimiento:
            ...
            with col1:
                comparison_type = st.selectbox(
                    ...
                    key="comparison_type_redac_vidae"
                )
            ...
            if comparison_type == "custom":
                ...
                with col1:
                    current_start = st.date_input(
                        "Inicio actual:",
                        value=datetime.now() - timedelta(days=7),
                        key="growth_current_start_redac_vidae"
                    )
                with col2:
                    current_end = st.date_input(
                        "Fin actual:",
                        value=datetime.now(),
                        key="growth_current_end_redac_vidae"
                    )
                ...
                with col3:
                    previous_start = st.date_input(
                        ...
                        key="growth_previous_start_redac_vidae"
                    )
                with col4:
                    previous_end = st.date_input(
                        "Fin comparación:",
                        value=datetime.now() - timedelta(days=8),
                        key="growth_previous_end_redac_vidae"
                    )
            ...
            if historical_df is not None and not historical_df.empty:
                ...
                if 'autor' in historical_df.columns and historical_df['autor'].notna().any():
                    ...
                    selected_author = st.selectbox(
                        "Seleccionar autor:",
                        available_authors,
                        key="selected_author_historical_vidae"
                    )
