...
st.set_page_config(
    page_title="Dashboard Bumeran - Redacción",
    page_icon="💼",
    layout="wide"
)

# Verificar login antes de mostrar contenido
if not check_login('bumeran', page_type='redaccion'):
    st.stop()

# Obtener configuración del medio
media_config = create_media_config()['bumeran']
...
if date_option == "Preestablecido":
    ...
    with col1:
        start_date_custom = st.date_input(
            ...
            key="start_date_bumeran"
        )
    with col2:
        end_date_custom = st.date_input(
            "Fecha fin:",
            value=datetime.now(),
            key="end_date_bumeran"
        )
    ...
    **Posibles causas:**
    ...
    if not sheets_filtered.empty and ga4_df is not None and not ga4_df.empty:
        ...
        with tab1:
            ...
            st.markdown(f"""
            ### 🎯 Objetivo del Mes
            **Meta:** 800,000 de Page Views{' desde ' + country_filter if country_filter != "Todos los países" else ''}
            ...
            monthly_goal = 800000  # 800 mil Page Views
        ...
        with tab2:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"bumeran_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv"
            )
            ...
            if historical_period == "custom":
                ...
                with col3:
                    hist_start_date = st.date_input(
                        ...
                        key="hist_start_bumeran"
                    )
                with col4:
                    hist_end_date = st.date_input(
                        "Fecha fin:",
                        value=datetime.now(),
                        key="hist_end_bumeran"
                    )
            else:
            ...
            if historical_df is not None and not historical_df.empty:
                ...
                if 'autor' in historical_df.columns and historical_df['autor'].notna().any():
                    ...
                    selected_author = st.selectbox(
                        "Seleccionar autor:",
                        available_authors,
                        key="selected_author_historical_bumeran"
                    )
        ...
        with tab7:
            ...
            period_filter = st.selectbox(
                ...
                key="period_filter_metrics_bumeran"
            )
            ...
            if period_filter == "custom":
                ...
                with col1:
                    custom_start_date = st.date_input(
                        ...
                        key="custom_start_metrics_bumeran"
                    )
                with col2:
                    custom_end_date = st.date_input(
                        "Fecha fin:",
                        value=datetime.now(),
                        key="custom_end_metrics_bumeran"  
                    )
