...
if sheets_filtered.empty and (ga4_df is None or ga4_df.empty):
    ...
    **Posibles causas:**
    ...
    if not sheets_filtered.empty and ga4_df is not None and not ga4_df.empty:
        ...
        tab1, tab2, tab4, tab_crecimiento, tab_historico = st.tabs(["📊 KPI", "📋 Datos", "🔝 Top Páginas", "📈 Crecimiento", "📈 Histórico"])
            ...
            if comparison_type == "custom":
                ...
                with col1:
                    current_start = st.date_input(
                        "Inicio actual:",
                        value=datetime.now() - timedelta(days=7),
                        key="growth_current_start_redac_ole"
                    )
                with col2:
                    current_end = st.date_input(
                        "Fin actual:",
                        value=datetime.now(),
                        key="growth_current_end_redac_ole"
                    )
                ...
                with col3:
                    previous_start = st.date_input(
                        ...
                        key="growth_previous_start_redac_ole"
                    )
                with col4:
                    previous_end = st.date_input(
                        "Fin comparación:",
                        value=datetime.now() - timedelta(days=8),
                        key="growth_previous_end_redac_ole"
                    )
