...
if sheets_filtered.empty and (ga4_df is None or ga4_df.empty):
    ...
    **Posibles causas:**
    ...
    if not sheets_filtered.empty and ga4_df is not None and not ga4_df.empty:
        ...
        top_n = st.slider("Número de URLs a mostrar:", 5, 50, 20, key="top_urls_slider_clarin")
        ...
        with col1:
            comparison_date_option = st.selectbox(
                ...
                key="comparison_date_range_clarin"
            )
        ...
        if comparison_date_option == "Personalizado":
            ...
            with col1:
                comparison_start_date = st.date_input(
                    "Fecha inicio:",
                    value=datetime.now() - timedelta(days=7),
                    key="comparison_start_clarin"
                )
            with col2:
                comparison_end_date = st.date_input(
                    "Fecha fin:",
                    value=datetime.now(),
                    key="comparison_end_clarin"
                )
        ...
        with col1:
            comparison_type = st.selectbox(
                ...
                key="comparison_type_cliente_clarin"
            )
        ...
        if comparison_type == "custom":
            ...
            with col1:
                current_start = st.date_input(
                    "Inicio actual:",
                    value=datetime.now() - timedelta(days=7),
                    key="growth_current_start_cliente_clarin"
                )
            with col2:
                current_end = st.date_input(
                    "Fin actual:",
                    value=datetime.now(),
                    key="growth_current_end_cliente_clarin"
                )
            ...
            with col3:
                previous_start = st.date_input(
                    ...
                    key="growth_previous_start_cliente_clarin"
                )
            with col4:
                previous_end = st.date_input(
                    "Fin comparación:",
                    value=datetime.now() - timedelta(days=8),
                    key="growth_previous_end_cliente_clarin"
                )
